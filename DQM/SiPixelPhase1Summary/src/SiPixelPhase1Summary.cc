...
void SiPixelPhase1Summary::bookTrendPlots(DQMStore::IBooker & iBooker){
  ...
  if (runOnEndLumi_){
    deadROCTrends_["bpix"] = iBooker.book1D("deadRocTrendBPix","BPIX dead ROC trend",500,0.,500);
    deadROCTrends_["bpix"]->setAxisTitle("Lumisection/10",1);
    deadROCTrends_["fpix"] = iBooker.book1D("deadRocTrendFPix","FPIX dead ROC trend",500,0.,500);
    deadROCTrends_["fpix"]->setAxisTitle("Lumisection/10",1);
    ineffROCTrends_["bpix"] = iBooker.book1D("ineffRocTrendBPix","BPIX inefficient ROC trend",500,0.,500);
    ineffROCTrends_["bpix"]->setAxisTitle("Lumisection/10",1);
    ineffROCTrends_["fpix"] = iBooker.book1D("ineffRocTrendFPix","FPIX inefficient ROC trend",500,0.,500);
    ineffROCTrends_["fpix"]->setAxisTitle("Lumisection/10",1);
  }
  else {
...
void SiPixelPhase1Summary::fillSummaries(DQMStore::IBooker & iBooker, DQMStore::IGetter & iGetter){
  ...
  for (int i = 0; i < 12; i++){ // !??!?!? xAxisLabels_.size() ?!?!
	...
	if (summaryMap_["Grand"]->getBinContent(i+1,j+1) > summaryMap_[name]->getBinContent(i+1,j+1)) summaryMap_["Grand"]->setBinContent(i+1,j+1,summaryMap_[name]->getBinContent(i+1,j+1)); // This could be changed to include warnings if we want?
      }
    }
  }
...
void SiPixelPhase1Summary::fillTrendPlots(DQMStore::IBooker & iBooker, DQMStore::IGetter & iGetter, int lumiSec){
  ...
  for (auto it : {1,2,3,4}){
    ...
    if (runOnEndLumi_) tempLayerME->Reset(); //If we're doing 
    if (it > 2) continue;
  ...
  //  std::cout << "Forward: " << nFilledROCsFPix << " " << hiEffROCsFPix << " Barrel: " << nFilledROCsBPix << " " << hiEffROCsBPix << std::endl;
  if (!runOnEndLumi_) { //offline
    deadROCTrends_["offline"]->Fill(0,18944-nFilledROCsBPix);
    deadROCTrends_["offline"]->Fill(1,10752-nFilledROCsFPix);
    ineffROCTrends_["offline"]->Fill(0,18944-hiEffROCsBPix);
    ineffROCTrends_["offline"]->Fill(1,10752-hiEffROCsFPix);
  }
  else { //online
    deadROCTrends_["fpix"]->Fill(lumiSec/10,10752-nFilledROCsFPix);
    deadROCTrends_["bpix"]->Fill(lumiSec/10,18944-nFilledROCsBPix);
    ineffROCTrends_["fpix"]->Fill(lumiSec/10,10752-hiEffROCsFPix);
    ineffROCTrends_["bpix"]->Fill(lumiSec/10,18944-hiEffROCsBPix);
  }
